import { RawDiff, DiffHunk, DiffHunkHeader, DiffLine, DiffLineType } from '../models/diff'
...
export class DiffParser {
   ...
   *             diffs.
   */
  public parse(text: string): RawDiff {
    ...
    try {
      ...
      // empty diff
      if (!headerInfo) { return new RawDiff([], false) }
      if (headerInfo.isBinary) { return new RawDiff([], true) }
      ...
      return new RawDiff(hunks, headerInfo.isBinary)
    } finally {
      this.reset()
    }
  }
