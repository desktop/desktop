...
export class AppStore {
  ...
  public constructor(gitHubUserStore: GitHubUserStore, cloningRepositoriesStore: CloningRepositoriesStore, emojiStore: EmojiStore, issuesStore: IssuesStore, statsStore: StatsStore, signInStore: SignInStore, accountsStore: AccountsStore, repositoriesStore: RepositoriesStore) {
  ...
  public async _repositoryWithRefreshedGitHubRepository(repository: Repository): Promise<Repository> {
    ...
    const withUpdatedGitHubRepository = updatedRepository.withGitHubRepository(gitHubRepository.withAPI(apiRepo))
    if (structuralEquals(withUpdatedGitHubRepository, repository)) {
      return withUpdatedGitHubRepository
    }

    return this.repositoriesStore.updateGitHubRepository(withUpdatedGitHubRepository)
  }
  ...
  public _deleteBranch(repository: Repository, branch: Branch): Promise<void> {
    return this.withAuthenticatingUser(repository, async (repository, account) => {
  ...
  private async performPush(repository: Repository, account: Account | null): Promise<void> {
    const gitStore = this.getGitStore(repository)
  ...
  /** This shouldn't be called directly. See `Dispatcher`. */
  private async performPull(repository: Repository, account: Account | null): Promise<void> {
    return this.withPushPull(repository, async () => {
      const gitStore = this.getGitStore(repository)
      const remote = gitStore.remote
  ...
  /** This shouldn't be called directly. See `Dispatcher`. */
  public async _publishRepository(repository: Repository, name: string, description: string, private_: boolean, account: Account, org: IAPIUser | null): Promise<Repository> {
    const api = API.fromAccount(account)
   ...
   *
   */
  public async _fetchRefspec(repository: Repository, refspec: string): Promise<void> {
    return this.withAuthenticatingUser(repository, async (repository, account) => {
      const gitStore = this.getGitStore(repository)
  ...
  public _updateRepositoryPath(repository: Repository, path: string): Promise<Repository> {
  ...
  public _updateRepositoryMissing(repository: Repository, missing: boolean): Promise<Repository> {
  ...
  public async _removeRepositories(repositories: ReadonlyArray<Repository | CloningRepository>): Promise<void> {
  ...
  private async refreshGitHubRepositoryInfo(repository: Repository): Promise<Repository> {
    const refreshedRepository = await this._repositoryWithRefreshedGitHubRepository(repository)
  ...
  public async _cloneAgain(url: string, path: string, account: Account | null): Promise<void> {
    ...
    if (found) {
      const updatedRepository = await this._updateRepositoryMissing(found, false)
