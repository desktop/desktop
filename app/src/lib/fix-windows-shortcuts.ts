...
export function fixAllWindowsShortcuts() {
  const shortcutPaths = [
    path.join(os.homedir(), 'AppData', 'Roaming', 'Microsoft', 'Windows', 'Start Menu', 'Programs', 'GitHub, Inc', 'GitHub Desktop.lnk'),
...
function fixWindowsShortcut(shortcutPath: string) {
  try {
    ...
    if (shortcutDetails.toastActivatorClsid === undefined || shortcutDetails.toastActivatorClsid === '') {
      return
    }

    console.info(`Found previous shortcut with toast activator CLSID in ${shortcutPath}. Fixing it...`)
