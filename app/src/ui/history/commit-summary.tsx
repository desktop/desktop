...
> {
  ...
  private renderDiffWarning = () => {
    ...
    console.log(shasInDiff)
    const excludedCommits = selectedCommits.filter(
      ({ sha }) => !shasInDiff.includes(sha)
    )
    console.log(excludedCommits)
    const countExcludedCommits = excludedCommits.length
    if (countExcludedCommits === 0) {
      return
    }

    const pluralized = countExcludedCommits > 1 ? 'commits' : 'commit'
    const diffWarning =
      `This diff will not contain changes from all your selected commits. ` +
      `The diff is generated by comparing the parent of the first commit and last commit in your selection and ` +
      `${countExcludedCommits} of the ${pluralized} in your selection do not share the same tree as those commits.`
