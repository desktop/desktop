...
> {
  ...
  private renderDiffWarning = () => {
    ...
    const diffWarning =
      `This diff will not contain changes from all your selected commits. ` +
      `The diff is generated by comparing the parent of the oldest commit and latest commit in your selection and ` +
      `${countExcludedCommits} of the commits in your selection do not share the same history as those commits. ` +
      `This happens when two branches are merged and the parent of the oldest commit and the latest commit are from only one of those two branches.`

    return (
      <TooltippedContent
        className="commit-exclusion-info"
        tooltip={diffWarning}
      >
