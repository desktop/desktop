...
function symlinkCLI(asAdmin: boolean) {
  ...
  exitCode = runas('/bin/mkdir', [ '-p', Path.dirname(InstalledCLIPath) ], { admin: asAdmin })
  if (exitCode !== 0) {
    throw new Error(`Failed to create intermediate directories to ${InstalledCLIPath}`)
  }

  exitCode = runas('/bin/ln', [ '-s', PackagedPath, InstalledCLIPath ], { admin: asAdmin })
  if (exitCode !== 0) {
