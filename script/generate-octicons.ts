...
/** Data for a specific height variation of an octicon. */
interface IOcticonHeightData {
  /** SVG path values for an icon. */
  readonly p: string[]
...
interface IOcticonData {
  ...
  /** Icon height variations, keyed by height as a string e.g. '16' */
  readonly v: Record<string, IOcticonHeightData>
}
...
generateIconData().then(({ names, sizes, results }) => {
  ...
  // Subsequent lines dedented for the correct indentation of the output file.
  out.write(`/*
...
export type OcticonSymbolType = {
  ...
  /** The symbol height variations, keyed by height */
  readonly v: OcticonSymbolHeights
}\n\n`)

  results.forEach(({ s, v }) => {
    const jsSafeName = getJsFriendlyName(s)
    const typeData = JSON.stringify({ s, v })
    out.write(`export const ${jsSafeName}: OcticonSymbolType = ${typeData}\n`)
  })
